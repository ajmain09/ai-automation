...
export default async function PageSettingsPage({ params }: { params: Promise<{ pageId: string }> }) {
  ...
  return <main className="workspace"><div className="page-heading"><div><div className="eyebrow"><Link href={`/pages/${page.id}`}>{page.name}</Link> / Settings</div><h1>Page settings</h1><p className="subtitle">Configure defaults and required order fields for this Page.</p></div></div><PageTabs pageId={page.id} active="Settings" /><PageSettingsForm pageId={page.id} initial={{ currency: page.settings?.currency ?? "USD", countryCode: page.settings?.countryCode ?? "US", defaultLanguage: page.settings?.defaultLanguage ?? "en", tone: page.settings?.tone ?? "helpful", requiredOrderFields: Array.isArray(page.settings?.requiredOrderFields) ? page.settings.requiredOrderFields.map(String) : ["name", "phone", "address", "product", "variant", "quantity"] }} /></main>;
}
