...
export default async function BusinessPage({ params }: { params: Promise<{ pageId: string }> }) {
  ...
  return <main className="workspace"><div className="page-heading"><div><div className="eyebrow"><Link href={`/pages/${page.id}`}>{page.name}</Link> / Business</div><h1>Business setup</h1><p className="subtitle">Changes create a page-scoped draft. Live configuration remains unchanged until publish.</p></div><span className="status-chip gray"><span className="dot" /> Draft-first workflow</span></div><PageTabs pageId={page.id} active="Business" /><BusinessEditor pageId={page.id} initial={{ businessName: data?.business_profile?.business_name ?? profile?.businessName ?? "", description: data?.business_profile?.description ?? profile?.description ?? "", benefits: data?.business_profile?.benefits?.join("\n") ?? toText(profile?.benefits), deliveryPolicy: data?.policies?.delivery ?? profile?.deliveryPolicy ?? "", codPolicy: data?.policies?.cod ?? profile?.codPolicy ?? "", faq: data?.policies?.faq?.join("\n") ?? toText(profile?.faq), salesInstructions: data?.sales_instructions ?? profile?.salesInstructions ?? "", notes: data?.order_requirements?.join("\n") ?? "", rawBusinessInfo: draft?.rawBusinessInfo ?? "" }} /></main>;
}
