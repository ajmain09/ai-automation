...
import { getPreviewTelegramSettings, getPreviewUsage } from "@/services/preview/store";
import { isDevPreview } from "@/lib/env";
import { PageControls } from "@/components/pages/page-controls";
import { PageTabs } from "@/components/pages/page-tabs";
import { PageTestButton } from "@/components/pages/page-test-button";
...
export default async function PageWorkspace({ params }: { params: Promise<{ pageId: string }> }) {
  ...
  if (!page) notFound();
  if (pageId !== page.slug) redirect(`/pages/${page.slug}`);
  const usage = isDevPreview() ? getPreviewUsage(page.id).month : { calls: 0, totalTokens: 0, estimatedCost: 0, estimatedCostBdt: 0 };
  ...
  const telegramConfigured = isDevPreview() ? getPreviewTelegramSettings(page.id).status === "CONNECTED" : ("telegramSettings" in page && page.telegramSettings?.status === "CONNECTED");
  return <main className="workspace">
    ...
    <section className="card operational-card"><div className="section-head"><div><h2>System status</h2><span className="muted">Live operating state for this Page</span></div></div><div className="status-grid"><StatusRow label="Facebook" value={page.connectionStatus === "CONNECTED" ? "Connected" : "Not connected"} /><StatusRow label="AI" value={page.aiStatus === "PAUSED_BY_BUDGET" ? "Paused by budget" : page.aiEnabled ? "Live" : "Paused"} /><StatusRow label="Business" value={live ? "Published" : "Draft"} /><StatusRow label="Products" value={`${activeProducts} active`} /><StatusRow label="Telegram" value={telegramConfigured ? "Connected" : "Not configured"} /><StatusRow label="AI Cost this month" value={`$${usage.estimatedCost.toFixed(6)} · ৳${(usage.estimatedCostBdt ?? usage.estimatedCost * 120).toFixed(2)}`} /></div></section>
    <div className="grid grid-2 overview-lower"><section className="card card-pad"><h2>Quick actions</h2><div className="quick-actions"><Link className="button secondary" href={`/pages/${page.slug}/business`}>Edit Business</Link><Link className="button secondary" href={`/pages/${page.slug}/products`}>Manage Products</Link><Link className="button secondary" href={`/pages/${page.slug}/ai`}>AI Settings</Link><PageTestButton pageId={page.slug} /></div></section><section className="card card-pad"><h2>Configuration</h2><div className="config-lines"><div><span>Live version</span><strong>{live ? `v${live.version}` : "Not published"}</strong></div><div><span>Draft</span><strong>{draft ? "Pending changes" : "No pending changes"}</strong></div><div><span>AI usage this month</span><strong>{usage.calls} calls · {usage.totalTokens.toLocaleString()} tokens</strong></div></div></section></div>
