...
import { getPageTelegramSettings, setPageTelegramDestination, testPageTelegramDestination } from "@/services/telegram/service";
...
export async function POST(request: Request, { params }: { params: Promise<{ pageId: string }> }) {
  ...
  if (parsed.data.action === "test") {
    if (!parsed.data.botToken) return NextResponse.json({ error: "Enter the bot token before testing Telegram." }, { status: 400 });
    const result = await testPageTelegramDestination({ pageId: resolvedPageId, botToken: parsed.data.botToken, chatId: parsed.data.chatId, newOrderEnabled: parsed.data.newOrderEnabled, updatedOrderEnabled: parsed.data.updatedOrderEnabled, cancelledOrderEnabled: parsed.data.cancelledOrderEnabled }, admin.id);
    return result.ok ? NextResponse.json(result) : NextResponse.json(result, { status: 422 });
  }
