...
import { getPreviewPage, updatePreviewSettings } from "@/services/preview/store";
...
export async function POST(request: Request, { params }: { params: Promise<{ pageId: string }> }) {
  ...
  const existing = await prisma.page.findFirst({ where: { OR: [{ id: pageId }, { slug: pageId }] }, select: { id: true } });
  if (!existing) return NextResponse.json({ error: "Page not found." }, { status: 404 });
  const settings = await prisma.$transaction(async (tx) => { const value = await tx.pageSettings.upsert({ where: { pageId: existing.id }, update: parsed.data, create: { pageId: existing.id, ...parsed.data } }); await tx.auditLog.create({ data: { adminId: admin.id, pageId: existing.id, action: "page.settings_updated" } }); return value; });
