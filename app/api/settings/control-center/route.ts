...
import { getMetaControlCenter, saveMetaPlatformConfig, testMetaPlatformConfig } from "@/services/meta/settings";

const inputSchema = z.object({
  section: z.enum(["meta", "testMeta", "pauseAllAi", "resumeAllAi"]),
...
export async function POST(request: Request) {
  ...
  if (input.section === "meta") return NextResponse.json({ ok: true, control: await saveMetaPlatformConfig({ appId: input.appId ?? "", appSecret: input.appSecret, verifyToken: input.verifyToken, graphApiVersion: input.graphApiVersion }, admin.id) });
