...
The production topology is `app`, `worker`, `postgres`, and `caddy` from `docker-compose.yml`. Redis and external automation services are not part of this system.

1. Provision a VPS with Docker/Compose, persistent PostgreSQL storage, and DNS for the domain.
2. Copy the repository and create a production environment file from `.env.example`; set every secret explicitly and never commit it.
...
6. Check worker logs, Caddy HTTPS, and PostgreSQL connectivity.
