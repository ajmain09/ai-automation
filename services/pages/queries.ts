...
export async function getDashboardData() {
  ...
  return { pages: pages.map((page) => ({ id: page.id, slug: page.slug, name: page.name, connectionStatus: page.connectionStatus, aiEnabled: page.aiEnabled, usage: usageByPage.get(page.id) ?? { calls: 0, totalTokens: 0, estimatedCost: 0, estimatedCostBdt: 0 } })) };
}
...
export async function getPageById(idOrSlug: string) {
  ...
  return prisma.page.findFirst({ where: { OR: [{ id: idOrSlug }, { slug: idOrSlug }] }, include: { settings: true, connection: true, businessProfile: true, products: { include: { variants: true }, orderBy: { createdAt: "desc" } }, configurationVersions: { orderBy: { version: "desc" } }, _count: { select: { customers: true, conversations: true } } } });
}
