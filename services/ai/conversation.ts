...
export async function processConversationJob(job: QueueJob, provider?: AiProvider) {
  ...
  const generationLiveConfiguration = await prisma.configurationVersion.findFirst({ where: { pageId: job.pageId, status: "LIVE" }, select: { version: true } });
  ...
  void updatedMemory;
  await sendSafeReply({ pageId: job.pageId, conversationId: conversation.id, recipientPsid: conversation.customer.facebookPsid, text: orderResult?.reply ?? effectiveResult.reply, generatedVersion: conversation.version, recommendedProductIds: orderResult?.orderTruth ? [] : effectiveResult.recommended_product_ids, generatedConfigurationVersion: generationLiveConfiguration?.version, currentOrderState: orderResult?.orderTruth, jobExpiresAt: job.expiresAt, outboundAttemptKey: `reply:${job.id}:${conversation.version}` });
}
