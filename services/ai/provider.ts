...
export class DeepSeekProvider implements AiProvider {
  ...
  constructor(private readonly apiKey = getEnv().DEEPSEEK_API_KEY, private readonly baseUrl = getEnv().DEEPSEEK_BASE_URL) { this.model = normalizeModel(getEnv().DEEPSEEK_MODEL).model; }
...
export async function beginProviderAttempt(input: { pageId: string; provider: AiProvider; callType: AiCallType; attemptNumber: number }): Promise<AttemptHandle> {
  ...
  const run = await prisma.$transaction(async (tx) => {
    ...
    await tx.apiUsage.create({ data: { pageId: input.pageId, aiAttemptId: created.id, provider: input.provider.name, model: input.provider.model, callType: input.callType, inputRateSnapshot: env.DEEPSEEK_INPUT_RATE, inputHitRateSnapshot: env.DEEPSEEK_INPUT_RATE, inputMissRateSnapshot: env.DEEPSEEK_INPUT_RATE, outputRateSnapshot: env.DEEPSEEK_OUTPUT_RATE, attemptNumber: input.attemptNumber, status: "RUNNING" } });
    return created;
  });
...
export async function finishProviderAttempt(input: { handle: AttemptHandle; usage?: ProviderUsage; status: string; error?: string }) {
  ...
  return prisma.$transaction(async (tx) => {
    const cost = calculateUsageCost({ inputTokens, cachedInputTokens: input.usage?.cachedInputTokens, outputTokens, inputCacheHitRateUsd: env.DEEPSEEK_INPUT_RATE, inputCacheMissRateUsd: env.DEEPSEEK_INPUT_RATE, outputRateUsd: env.DEEPSEEK_OUTPUT_RATE, usdBdtRate: 1 });
    ...
    return tx.apiUsage.update({ where: { aiAttemptId: input.handle.runId }, data: { inputTokens, cachedInputTokens: input.usage?.cachedInputTokens, outputTokens, totalTokens, providerUsageJson: input.usage?.raw as never, estimatedCost: cost.totalUsd, estimatedCostUsd: cost.totalUsd, estimatedCostBdt: cost.totalBdt, costEstimated: cost.estimated, providerRequestId: input.usage?.requestId, status: input.status, latencyMs: duration } });
  });
}
